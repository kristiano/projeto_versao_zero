...
def adaptar_material(dimensoes: dict, assunto: str, texto: str) -> str:
    ...
    rewrite_sys_msg = (
        ...
        "2. **Eixo de Entrada:**\n"
        "   - Se **Visual**: Crie um diagrama, mapa conceitual ou fluxograma real usando a linguagem gráfica **Mermaid**. Você deverá anexar a arquitetura visual através de código cru em blocos ```mermaid...``` para que sejam compilados pelo meu renderizador ao invés de usar 'exemplos mentais' em texto. IMPORTANTE: Use apenas tipos suportados estritamente pelo Mermaid oficial (ex: `graph TD`, `mindmap`, `pie` ou `classDiagram`). Não invente sintaxes (ex: `boxplot`), abstraia o conceito em um fluxograma de hierarquia ou mapa mental!\n"
